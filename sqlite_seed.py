...
def seed_data(conn: sqlite3.Connection):
    ...
    vendors = [
        ...
        ("VEND-GLOB", "Global Industrial", "hello@glob.test", "+1-415-000-0000", "200 Market St", "San Francisco", "USA"),
        ("VEND-OMEG", "Omega Tools", "contact@omega.test", "+44-20-0000-0001", "10 Fleet St", "London", "UK"),
        ("VEND-BER1", "Berliner Technik", "verkauf@ber1.test", "+49-30-0000-0002", "Unter den Linden 5", "Berlin", "Germany"),
        ...
        ("VEND-PACF", "Pacific Gear", "sales@pacific.test", "+1-650-000-0005", "600 Embarcadero", "San Francisco", "USA"),
    ]
    cur.executemany(
        """
    ...
    # Assets
    assets = [
        ("AST-0001", "Forklift 1", nyc_id, nyc_a1_id, "SN-FL-001", "Vehicle", "Active", 12500.00, date(2024, 5, 1).isoformat(), acme_id),
        ("AST-0002", "Conveyor A", sfo_id, sfo_b1_id, "SN-CNV-101", "Equipment", "Active", 8500.00, date(2024, 6, 15).isoformat(), glob_id),
        ("AST-0003", "Pallet Jack", nyc_id, nyc_wh_id, "SN-PJ-900", "Tool", "Maintenance", 600.00, date(2024, 7, 20).isoformat(), acme_id),
    ]
    ...
    # add 8 more bills
    more_bills_vendors = [omega_vendor_id, ber1_vendor_id, tok1_vendor_id, nova_vendor_id, pacf_vendor_id, acme_id, glob_id, omega_vendor_id]
    ...
    for i in range(6):
        ...
        pos.append((po_num, po_vendors[i], date(2024 + (i % 2), (i % 12) + 1, (i % 27) + 1).isoformat(), "Open" if i % 2 == 0 else "Closed", po_sites[i]))
    cur.executemany(
        """
    ...
    po_numbers = [row[0] for row in cur.execute("SELECT PONumber FROM PurchaseOrders WHERE PONumber NOT IN ('PO-10001','PO-10002') ORDER BY PONumber").fetchall()]
    ...
    for i in range(8):
        ...
        sos.append((so_num, so_customers[i], date(2024 + (i % 2), (i % 12) + 1, (i % 27) + 1).isoformat(), "Open" if i % 3 else "Closed", so_sites[i]))
    cur.executemany(
        """
    ...
    so_numbers = [row[0] for row in cur.execute("SELECT SONumber FROM SalesOrders WHERE SONumber NOT IN ('SO-50001','SO-50002') ORDER BY SONumber").fetchall()]
    so_item_codes = [
